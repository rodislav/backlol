...
import {API_HOST, API_VERSION} from "../../App";
...
// noinspection JSMethodCanBeStatic
    ...
    get(url: string, onLoaded: (data: any) => {}): ReplaySubject {
        this.getJson(url)
            .subscribe(
                data => onLoaded(data),
    ...
    delete(...url: string | string []): ReplaySubject {
        return this._doFetch(this._createDeleteRequest(url))
    }

    post(body, ...url: string | string []): ReplaySubject {
        return this._doFetch(this._createPostRequest(body, url))
    }

    getJson(...url: string | string []): ReplaySubject {
        return this._doFetch(this._createGetRequest(url))
    }

    getText(...url: string | string []): ReplaySubject {
